    ...
    public final class ContentNode: ASDisplayNode {
        ...
        public func setPeer(
            ...
            if updatedState != self.state || overrideImage != self.overrideImage || theme !== self.theme {
                ...
                if let peer = peer, let signal = peerAvatarImage(postbox: postbox, peerReference: PeerDisplayOverlay.mediaPeerReference(for: peer) ?? PeerReference(peer), authorOfMessage: authorOfMessage, representation: representation, displayDimensions: displayDimensions, clipStyle: clipStyle, emptyColor: emptyColor, synchronousLoad: synchronousLoad, provideUnrounded: storeUnrounded, cutoutRect: cutoutRect) {
                    self.contents = nil
        ...
        func setPeerV2(
            ...
            if let imageCache = genericContext.imageCache as? DirectMediaImageCache, let peer, let smallProfileImage = peer.smallProfileImage, let peerReference = PeerDisplayOverlay.mediaPeerReference(for: peer) ?? PeerReference(peer) {
        ...
        public func setPeer(
            ...
            if updatedState != self.state || overrideImage != self.overrideImage || theme !== self.theme {
                ...
                if let peer = peer, let signal = peerAvatarImage(account: account, peerReference: PeerDisplayOverlay.mediaPeerReference(for: peer) ?? PeerReference(peer), authorOfMessage: authorOfMessage, representation: representation, displayDimensions: displayDimensions, clipStyle: clipStyle, emptyColor: emptyColor, synchronousLoad: synchronousLoad, provideUnrounded: storeUnrounded, cutoutRect: cutoutRect) {
                    self.contents = nil
                    self.displaySuspended = true
                    self.imageReady.set(self.imageNode.contentReady)
                    self.imageNode.setSignal(signal |> beforeNext { [weak self] next in
