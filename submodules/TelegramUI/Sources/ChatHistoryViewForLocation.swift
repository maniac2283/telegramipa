...
func preloadedChatHistoryViewForLocation(_ location: ChatHistoryLocationInput, context: AccountContext, chatLocation: ChatLocation, subject: ChatControllerSubject?, chatLocationContextHolder: Atomic<ChatLocationContextHolder?>, fixedCombinedReadStates: MessageHistoryViewReadState?, tag: HistoryViewInputTag?, additionalData: [AdditionalMessageHistoryViewData], orderStatistics: MessageHistoryViewOrderStatistics = []) -> Signal<ChatHistoryViewUpdate, NoError> {
    ...
    let isSimulatedChat = isSimulatedChatLocation(chatLocation)
...
private func isSimulatedChatLocation(_ chatLocation: ChatLocation) -> Bool {
    guard case let .peer(peerId) = chatLocation else {
        return false
    }
    return MessageSimulationOverlay.isSimulatedPeer(peerId)
}

private func chatHistoryShouldKeepLoading(view: MessageHistoryView, isSimulated: Bool) -> Bool {
    if isSimulated {
        return false
    }
    return view.isLoading || (view.entries.isEmpty && (view.holeEarlier || view.holeLater))
}
...
func chatHistoryViewForLocation(
    ...
    if scheduled {
        ...
        |> map { view, updateType, initialData -> ChatHistoryViewUpdate in
            ...
            if chatHistoryShouldKeepLoading(view: view, isSimulated: isSimulatedChat) {
        ...
        let trackHoles = !isSimulatedChat
        let clipHoles = !isSimulatedChat
        ...
        switch location.content {
            case let .Initial(count):
                ...
                var requestAroundId = isSimulatedChat
                ...
                } else if case let .peer(id) = chatLocation, id.namespace == Namespaces.Peer.CloudUser {
                    ...
                    if preloaded {
                        ...
                        if let maxReadIndex = view.maxReadIndex, tag == nil, canScrollToRead {
                            ...
                            if let _ = chatLocation.peerId {
                                ...
                                if !isSimulatedChat, minIndex <= 0 && view.holeEarlier {
                                    fadeIn = true
                                    return .Loading(initialData: combinedInitialData, type: .Generic(type: updateType))
                                }
                                if !isSimulatedChat, maxIndex >= view.entries.count {
                                    if view.holeLater {
                                        fadeIn = true
                                        return .Loading(initialData: combinedInitialData, type: .Generic(type: updateType))
                                    }
                            ...
                            if !isSimulatedChat, let _ = chatLocation.peerId, !effectiveIsAddedToChatList {
                                if view.holeEarlier && view.entries.count <= 2 {
                                    fadeIn = true
                                    return .Loading(initialData: combinedInitialData, type: .Generic(type: updateType))
                                }
                            }
                            if !isSimulatedChat, view.entries.isEmpty && (view.holeEarlier || view.holeLater) {
                                fadeIn = true
                                return .Loading(initialData: combinedInitialData, type: .Generic(type: updateType))
                            }
                        }
            ...
            case let .InitialSearch(searchLocationSubject, count, highlight, setupReply):
                ...
                case let .index(index):
                    signal = account.viewTracker.aroundMessageHistoryViewForLocation(context.chatLocationInput(for: chatLocation, contextHolder: chatLocationContextHolder), ignoreMessagesInTimestampRange: ignoreMessagesInTimestampRange, ignoreMessageIds: ignoreMessageIds, index: .message(index), anchorIndex: .message(index), count: count, trackHoles: trackHoles, clipHoles: clipHoles, ignoreRelatedChats: ignoreRelatedChats, fixedCombinedReadStates: nil, tag: tag, appendMessagesFromTheSameGroup: appendMessagesFromTheSameGroup, orderStatistics: orderStatistics, additionalData: additionalData, useRootInterfaceStateForThread: useRootInterfaceStateForThread)
                case let .id(id):
                ...
                return signal |> map { view, updateType, initialData -> ChatHistoryViewUpdate in
                    ...
                    if preloaded {
                        ...
                        if !view.entries.isEmpty {
                            ...
                            if !isSimulatedChat, maxIndex == view.entries.count && view.holeLater {
                                fadeIn = true
                                return .Loading(initialData: combinedInitialData, type: .Generic(type: updateType))
                            }
                        } else if !isSimulatedChat, view.holeEarlier || view.holeLater {
                            fadeIn = true
                            return .Loading(initialData: combinedInitialData, type: .Generic(type: updateType))
                        }
            ...
            case let .Scroll(subject, anchorIndex, sourceIndex, scrollPosition, animated, highlight, setupReply):
                ...
                var first = true
                return account.viewTracker.aroundMessageHistoryViewForLocation(context.chatLocationInput(for: chatLocation, contextHolder: chatLocationContextHolder), ignoreMessagesInTimestampRange: ignoreMessagesInTimestampRange, ignoreMessageIds: ignoreMessageIds, index: subject.index, anchorIndex: anchorIndex, count: 128, trackHoles: trackHoles, clipHoles: clipHoles, ignoreRelatedChats: ignoreRelatedChats, fixedCombinedReadStates: fixedCombinedReadStates, tag: tag, appendMessagesFromTheSameGroup: appendMessagesFromTheSameGroup, orderStatistics: orderStatistics, additionalData: additionalData, useRootInterfaceStateForThread: useRootInterfaceStateForThread)
                |> map { view, updateType, initialData -> ChatHistoryViewUpdate in
                    let (cachedData, cachedDataMessages, readStateData, _) = extractAdditionalData(view: view, chatLocation: chatLocation)
